...
if __name__ == '__main__':
    ...
    print('np.c_head',iris_dataset[:5])
    print('np.c_tail',iris_dataset[-5:])

    # iris_dataset[0], iris_dataset[50], iris_dataset[100]
    summaries = summarize_by_class(iris_dataset)  # 1 통계량요약
    probabilities = calculate_class_probability(summaries, iris_dataset[0])  # 각 데이터셋에 대하여 class prob. 계산
    print('probabilities3:',probabilities)
    probabilities = calculate_class_probability(summaries, iris_dataset[50])
    print('probabilities4:',probabilities)
